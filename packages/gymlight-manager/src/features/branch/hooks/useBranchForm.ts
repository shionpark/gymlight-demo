...
export const useBranchForm = () => {
  ...
  const onValidUpdateBranch = (branchId: number) => {
    return async (form: IBranchFormFields) => {
      ...
      const { openedDateYear, openedDateMonth, openedDateDay, name, code, tel, address } = form;

      const openDate = formatYYYYMMDD(openedDateYear, openedDateMonth, openedDateDay);
